cfrom flask import Flask, render_template, request, redirect, session, url_for, flash, send_file, jsonify
...
def signup():
    if request.method == 'POST':
        ...
        role = request.form.get('role', 'user')  # <-- New line
        ...
        if connection:
            try:
                ...
                cursor.execute(
                    ...
                    (username, password, role)  # <-- Role added here
                )
                connection.commit()
